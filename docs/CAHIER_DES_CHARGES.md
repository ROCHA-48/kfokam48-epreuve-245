...
| **Relecteur** (un étudiant, désigné au hasard parmi les présents pour relire l'exercice d'un pair — Q7) | Rendre une note (0–20, entière) et un commentaire (Q9) · Corriger sa note tant que la session n'est pas clôturée (Q10, voir la contradiction C1 en section 7) | Relire son propre exercice (Q5) · Relire plusieurs fois le même exercice (un seul relecteur — Q6) · Rendre ou corriger une relecture après la clôture de la session (RG14) |
...
|---|---|---|---|---|
| Z1 | Que se passe-t-il si aucun relecteur n'est disponible : un seul étudiant présent à la session, ou tous les autres présents sont l'auteur de l'exercice ? | Aucune des 16 questions ne couvre ce cas (c'est le trou du sujet) | L'exercice passe au statut « en attente d'assignation », distinct de « en attente de relecture » ; le système retente l'assignation à chaque nouvelle présence marquée sur la session (RG13) | L'exercice n'est jamais bloqué ni supprimé, le formateur le voit comme non assigné (Q11). Une issue dédiée, priorité Must. |
...
Les corrections de l'étape 3 (enveloppe) seront ajoutées ici, dans un commit qui dit explicitement ce qui est devenu faux.
